...
class Paradraw():
    def __init__(self,marks=['-'], colors = ['k'], markers = [''], thickness=[1],x_label = 'axis 1',y_label = 'axis 2',z_label = 'axis 3',c_label = 'colors', colmap = 'hot_r',xlim = False,ylim = False,zlim = False,clim = False, x_scale = 'linear',y_scale = 'linear',z_scale = 'linear',c_scale = 'linear',xlintresh=1.e-10,ylintresh=1.e-10,zlintresh=1.e-10,clintresh=1.e-10,title='',iscolorbar = False,fontsize = 28,ticksize = 22, markerssize = [3], x_tick_label = False,y_tick_label = False,z_tick_label = False,cbar_tick_label = False,ax_x_format = '%.2f',ax_y_format = '%.2f',ax_z_format = '%.2f',cbformat = '%.2f',figure=False, bar_col = [0.0,0.0,1.0],transparancy = False,ncont = 15,pcol_type = 'contourf',tight_layout=True, stem = False,legend=[False],legloc = 'best',axis=None,antialiasing=False,levels = False):
...
def plot2(data,param=False):
    ...
    if param.figure is False:
        ...
        ax = param.figure[1]
    if param.markeredge is False:
        if param.stem is False:ax.plot( x, y, linestyle = param.marks[0], marker = param.markers[0], ms = param.markerssize[0], color = param.colors[0],markeredgecolor='none',linewidth = param.thickness[0],label=param.legend[0])
        else:ax.stem( x, y, linestyle = param.marks[0], marker = param.markers[0], ms = param.markerssize[0], color = param.colors[0], markeredgecolor='none',linewidth = param.thickness[0],label=param.legend[0])
    else:
        if param.stem is False:ax.plot( x, y, linestyle = param.marks[0], marker = param.markers[0], ms = param.markerssize[0], color = param.colors[0], linewidth = param.thickness[0],label=param.legend[0])
        else:ax.stem( x, y, linestyle = param.marks[0], marker = param.markers[0], ms = param.markerssize[0], color = param.colors[0], linewidth = param.thickness[0],label=param.legend[0])
    # set the limits of the plot to the limits of the data
    param.fig=fig
    param.ax = ax
    Options((x,y),param)
...
#        else:
    if param.markeredge is False:
        ax.plot(x, y, z, linestyle = sty, color = ci, marker = mi, ms = ms, markeredgecolor='none',label=param.legend[0])
    else:
        ax.plot(x, y, z, linestyle = sty, color = ci, marker = mi, ms = ms, label=param.legend[0])
    param.fig = fig
    param.ax = ax
    Options((x,y,z),param)
...
#        else:
        if param.markeredge is False:
            ax.plot(x[j], y[j ], z[j],linestyle = sty, marker = mi, ms=ms, color = ci, markeredgecolor='none',label=leg)
        else:
            ax.plot(x[j], y[j], z[j], linestyle = sty, marker = mi, ms = ms, color = ci,label=leg)
