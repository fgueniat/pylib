...
def routh_array(pol):
    ...
    i_col = 0
    for i_n in range(n):
        A[i_row,i_col] = pol[i_n]
        if shift_column is False:
            shift_column = True
            i_row = i_row + 1
        else:
            shift_column = False
            i_row = i_row - 1
            i_col = i_col + 1
    for i_r in range(2,n):
        for i_c in range(m):
            a=A[i_r-1,0]
            if a==0:
                print('marginal stability')
                return A[:,0]
            else:
                mat = np.zeros((2,2))
                mat[:,0]=A[i_r-2:i_r,0]
                mat[:,1]=A[i_r-2:i_r,i_c+1]
            A[i_r,i_c] = -np.linalg.det(mat)/a
