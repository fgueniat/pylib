...
def cluster_doc(doc=None,grid_x=None,grid_y=None,nx=None,ny=None,nclust=10,renorm=True,recluster=False,ncluster_recluster = False):
    ...
    print(docs_t)
    docs = np.array([ docs_t[perm[key]] for key in xrange(nclust)])
    clusters_max = np.array([ clusters_max_t[perm[key]] for key in xrange(nclust)]).astype(int)
